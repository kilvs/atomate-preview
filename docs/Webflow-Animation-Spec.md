...
| Hero flow drop (Eugene, 17 Sep) | Once, when `.home_hero_flow` reaches 85% of the viewport (on load on desktop) | After 0.45s the label fades in. The five `.home_hero_flow_doc` pills then drop from −150px, bottom card first, 0.17s apart. Each fades in over 0.18s and falls with `bounce.out` over 0.85s, settling from a ±4° tilt. Then arrow 1 fades down, `.home_hero_flow_engine` pops from 0.92 (`back.out(1.6)`), arrow 2 fades down, and the output card rises 16px, its ticks sliding in 0.08s apart. It runs about 4.5s. GSAP timeline in `motion.js` §3b; in Webflow this is page custom code, not IX2. Under reduced motion the graphic is simply shown |
